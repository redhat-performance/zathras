...
gl_valid_os_vendors="rhel ubuntu amazon none"
...
# Arg1: If null, simple exit out, else text string of error
# Arg2: The exit code
...
{
	cleanup_install_lock
	if [[ $gl_cli_supplied_options != "" ]]; then
		rm $gl_cli_supplied_options 2> /dev/null
	fi
	if [[ $1 != "" ]]; then
		if [[ $3 != "" ]]; then
			sysname=${3}
		else
			sysname=${gl_host_config}
		fi
		timestamp=`date`
		if [[ ${sysname} != "" ]]; then
			error_string="${timestamp} ${sysname}, $1"
		else
			error_string="${timestamp} $1"
		fi
		echo -e ${error_string}
		if [[ $2 -eq 1 ]]; then
			flock -x $gl_top_dir/failed_runs echo -e "${error_string}" >> $gl_top_dir/failed_runs
		fi
	fi
	if [[ $gl_scenario_to_restore != "" ]]; then
		rm $gl_scenario_to_run
		mv $gl_scenario_to_restore $gl_scenario_to_run
	fi
	exit $2
}
...
show_aws_images()
{
	if [[ $gl_os_vendor == "ubuntu" ]]; then
		...
		cleanup_and_exit "Unknown os vendor $gl_os_vendor, valid types are ubuntu, rhel, amazon" 1 ${gl_system_type}
	fi
	end_date=`date +"%Y"`
	let "start_date=$end_date-1"
	if [[ $2 -eq 0 ]]; then
...
#
# Pull the version information for the git repository (using tags).  At this time
# we clone the repo, and then grab the git information.  We have not found a way
# yet on how to pull the tag information without cloning the git.
#
retrieve_versions()
{
	file=$1.yml
	if  test -f "$file" ; then
		location=`grep location: ${file} | cut -d: -f2- | sed "s/ //g"`
		if [[ $location == "https://github.com"* ]]; then
			#
			# Only way to show the versions with tags is to clone the repo
			#
			temp_dir=`mktemp -d`
			...
			if [ $? -ne 0 ]; then
				cleanup_and_exit "Unable to clone repo $repo" 1
			fi
			cd *
			echo ============================
			echo $1
...
show_test_version()
{
	...
	for test_name in $gl_test_list; do
		create_test_run_file $test_name
		cd tests_to_run
...
set_user_name()
{
	if [[ $gl_system_type == "aws" ]]; then
		if [[ $gl_os_vendor == "ubuntu" ]]; then
			echo "  test_user: ubuntu" >> ansible_vars_main.yml
		else
			echo "  test_user: ec2-user" >> ansible_vars_main.yml
		fi
	elif [[ $gl_system_type == "azure" ]]; then
		# Azure normally defaults to the running username
		echo "  test_user: ${user_name}" >> ansible_vars_main.yml
	elif [[ $gl_system_type == "gcp" ]]; then
		echo "  test_user: ${user_name}" >> ansible_vars_main.yml
	fi
	if [[ $gl_system_type == "local" ]]; then
		echo "  test_user: root" >> ansible_vars_main.yml
	fi
}
...
azure_specific_os_version()
{
	...
	#
	azpublisher=$(echo $gl_cloud_os_version | cut -f 1 -d:)
...
generate_tags()
{
  ...
  echo $gl_test_def_dir
	...
	if [[ $gl_system_type == "azure" ]]; then
		echo "resource \"azurerm_resource_group\" \"resource_group\" {" >> add_main_vars.tf
    		echo "    name     = var.resource_group"  >> add_main_vars.tf
	...
	if [[ $gl_system_type == "azure" ]]; then
		echo "    }" >> add_main_vars.tf
		echo "    timeouts {" >> add_main_vars.tf
		echo "        delete = \"15m\"" >> add_main_vars.tf
		echo "    }" >> add_main_vars.tf
	fi
	echo "}" >> add_main_vars.tf
}
...
create_ansible_options()
{
	...
	do
		...
		if [ $gl_preflight -eq 0 ]; then
			base_string="-t $gl_top_dir -d $run_dir -f $gl_sys_file -a $gl_create_attempts -S $gl_spot_recover -c $gl_test_def_dir $rm_dirs "
			if [[ "$gl_ssh_key_file" != "" ]]; then
		...
		if [ -d "tests_to_run" ]; then
			...
			do
				...
				if [[ ${results_file} == "" ]]; then
					...
					cd junk
					results_report=`tar xvf ../${results_file} | grep test_results_report`
...
#
# Pull the git repo branches.  Once we have that determine if the repo actually exist
# as well as the designated tag.  If either of them fail, report the error and bail out.
# If they both are ok, compare the versions and report as warnings any test that is using an
# outdated tag.
#
# $1: points to the git repo using.
# $2: gives the tag that is being used.
#
#
pull_git_repo()
{
	git_repo=${1}
	git_version=$2
	info_file=$(mktemp /tmp/zath_temp_git_info.XXXXXX)

	#
	# If the git_repo is not present we will get prompted for login information.
	# To get around this, run the command via the timeout.
	#
	timeout $gl_git_timeout git ls-remote $git_repo  1> $info_file 2> /dev/null

	#
	# If the info_file is empty, then the repo does not exist, or we can not connect
	# to the git repo.
	#
	if [[ ! -s $info_file ]]; then
		#
		# We may have been prompted for user name via the git ls-remote and any
		# new text will appear on that line.  Simply echo a new line out to make
		# the text appear on a line by itself.
		#
		cleanup_and_exit "\nError: $git_repo ls-remote request timed out.\n  Is there an issue with access to the git repo, or\n  perhaps the requested repo does not exist?" 1
	fi

	#
	# Check that the tag is present.
	#
	tag_looking_for="refs/tags/${2}"
	grep ${tag_looking_for}$ $info_file > /dev/null
	if [[ $? -ne 0 ]]; then
		cleanup_and_exit "Error: Tag ${2} not found in repo ${1}" 1
	fi
	#
	# Check for latest version, if not, flag the warning. Due to the JSON format, we
	# do not use this to make sure the tag is present or latest.
	#
	curl $git_repo/-/tags > $info_file 2> /dev/null
	latest_git_tag=`grep "div data-link" $info_file | head -n 1 | cut -d'{' -f 2 | cut -d';' -f 8 | cut -d'&' -f 1 | cut -d'-' -f 3 | sed "s/.zip//g"`

	if [[ $latest_git_tag != $git_version ]]; then
		echo Warning: Latest tag version for $git_repo is $latest_git_tag, you are using ${git_version}
		gl_warning_string="${gl_warning_string}\nWarning: Latest tag version for $git_repo is $latest_git_tag, you used $git_version"
	fi
...
verify_test_configuration()
{
	...
	if [[ $gl_test_version_check -eq 1 ]] || [[ $gl_update_test_versions -eq 1 ]]; then
		...
		for test_repo in `ls *yml`; do
			...
			if [[ $location == "https://github.com"* ]]; then
				...
				git clone $repo 2> /dev/null
				if [ $? -ne 0 ]; then
					cleanup_and_exit "1 Unable to clone repo $repo" 1
				fi
				cd *
				#
				...
				#
				version_fail=""
				git tag | grep ${using_version} > /dev/null
				if [ $? -ne 0 ]; then
	...
	#
	grep "^    upload_extra:" $config_file | cut -d':' -f 2 | sed "s/^ //g" > $verify_tmp_file
	while IFS= read -r fline
	do
		if [[ $fline == \""none\"" ]]; then
			continue
		fi
		if [[ $fline == \""<replace with kit location>\"" ]]; then
			continue
		fi
		file_check=`echo $fline | sed "s/\"//g" | sed "s/,/ /g"`
