...
General options
  ...
  --system_type &lt;vendor&gt; amazon, azure, gcp or local
...
Cloud options only
  --cloud_os_id &lt;os id&gt;: Image of the OS to install (example amazon aminumber)
...
----
$ burden --os_vendor rhel --show_os_versions --system_type amazon
...
--system_type amazon --tests linpack --host_config "m5.xlarge"
----
Run fio on AWS i3en.xlarge instance.  Note this assumes the fio test is defined in the config directory.
----
$ ./burden --ssh_key_file /home/test_user/permissions/aws_region_2_ssh_key --terminate_cloud 1 \
--cloud_os_id ami-0fdea47967124a409 --os_vendor rhel --results_prefix just_a_test \
--system_type amazon --tests fio --host_config "i3en.xlarge:Disks;type=internal"
----
Run uperf on AWS m5.xlarge. Note this assumes the uperf test is configured in the config directory.
----
$ ./burden --ssh_key_file /home/test_user/permissions/aws_region_2_ssh_key --terminate_cloud 1 \
--cloud_os_id ami-0fdea47967124a409 --os_vendor rhel --results_prefix just_a_test --system_type amazon \
...
$ ./burden --ssh_key_file /home/test_user/permissions/aws_region_2_ssh_key --terminate_cloud 1 \
--cloud_os_id ami-0fdea47967124a409 --os_vendor rhel --results_prefix just_a_test \
--system_type amazon --tests uperf,fio \
...
----
$ ./burden --scenario rhel_85_2 --system_type amazon --scenario_vars aws_vars
----
=== Test scenario files
...
file is used. The file contains variables that are prefixed with the the system type (amazon, azure, gcp or
...
++++
To run Zathras and have the values in the scenario file updated for an amazon run, the command is +
----
$ ./burden --scenario <scenario file> --scenario_vars <scenario vars file> --system_type amazon
----
...
* --sys_type : At this time will be one of: amazon, azure, gcp or local.
...
# to_sys_type: for results info, basically amazon, azure or local
...
----
$ ./burden --show_os_versions --os_vendor rhel --system_type amazon | egrep "RHEL-9.*x86"
...
----
$ cd testing/rhel/amazon/m5.xlarge_0
$ ls res*str*tar
...
----
global:
  ssh_key_file: <your ssh key for the amazon system>
  terminate_cloud: 1
  cloud_os_id: [os_id]
  os_vendor: rhel
  results_prefix: testing
...
----
$ ./burden --scenario aws_example --scenario_vars config/scenario_example --system_type amazon
----
