...
General options
  ...
  --mode &lt;mode&gt;: Selects the default mode for an OS. Default is to treat it as a normal package-managed OS,
    `image` treats it as a bootc-based host which cannot install packages and the default kit_upload_directory is $HOME/zathras_uploads.
...
Available tests
 ...
 etcd_fio:  etcd simulation via fio
 fio:  straight fio
