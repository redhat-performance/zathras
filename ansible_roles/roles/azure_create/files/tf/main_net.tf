...
resource "azurerm_linux_virtual_machine" "virtualmachine" {
    count                 = var.vm_count
    name                  = "${var.run_label}-vm-${format("%02d",count.index)}"
    location              = var.location
    resource_group_name   = azurerm_resource_group.resource_group.name
    network_interface_ids = ["${azurerm_network_interface.publicnic[count.index].id}","${azurerm_network_interface.testnic1[count.index].id}"]
    ...
    os_disk {
        name                 = "${var.run_label}-OS_Disk-${format("%02d",count.index)}"
        caching              = "ReadWrite"
